/* global cy, Cypress */
/// <reference types="Cypress" />
const env = Cypress.env();
import { qase } from 'cypress-qase-reporter/dist/mocha';
import HomePage from '../../support/pageobjects/HomePage';

describe('Alerts And Modals Tests', function () {

    const home = new HomePage();

    qase([34],
    it('should visit the web and click on the Bootstrap Alerts Link', function() {
        home.visit();
        home.isReady();
        home.closePopUp();
        const bootstrapAlerts = home.clickOnBootstrapAlertsLinkHeader();
        bootstrapAlerts.isReady();
    }));

    qase([35],
    it('should visit the web and click on the Bootstrap Modals Link', function() {
        home.visit();
        home.isReady();
        home.closePopUp();
        const bootstrapModals = home.clickOnBootstrapModalsLinkHeader();
        bootstrapModals.isReady();
    }));

    qase([36],
    it('should visit the web and click on the File Download Link', function() {
        home.visit();
        home.isReady();
        home.closePopUp();
        const fileDownload = home.clickOnFileDownloadLinkHeader();
        fileDownload.isReady();
    }));

    qase([37],
    it('should visit the web and click on the Javascript Alerts Link', function() {
        home.visit();
        home.isReady();
        home.closePopUp();
        const javascriptAlerts = home.clickOnJavascriptAlertsLinkHeader();
        javascriptAlerts.isReady();
    }));

    qase([38],
    it('should visit the web and click on the Progress Bar Modal Link', function() {
        home.visit();
        home.isReady();
        home.closePopUp();
        const progressBarModal = home.clickOnProgressBarModalLinkHeader();
        progressBarModal.isReady();
    }));
    
    qase([39],
    it('should visit the web and click on the Windows Popup Modal Link', function() {
        home.visit();
        home.isReady();
        home.closePopUp();
        const windowsPopupModal = home.clickOnWindowPopupModalLinkHeader();
        windowsPopupModal.isReady();
    }));
});